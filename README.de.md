...
make sandbox-images-pull        # die zwei fertigen Arbeitsplätze, in denen ein Agent läuft
...
**Sandbox-Isolation.** Die Control Plane startet Sandboxes als Container (**Docker-Provider**, die Vorgabe) — echte Isolation auf Container-Ebene. `make sandbox-image` baut das Profil `base` ([`Dockerfile.sandbox`](Dockerfile.sandbox)), `make sandbox-image-dev` das Profil `dev` ([`Dockerfile.sandbox.dev`](Dockerfile.sandbox.dev)). **Das Image hängt am Agenten**, nicht an der Instanz: Ein Support- oder Mail-Agent läuft auf `base` und schleppt die JVM eines Entwickler-Agenten nicht mehr mit. Das Profil wird je Agent in der Oberfläche gesetzt, ein eigenes Image ist dort ein gültiger Wert. `COVEY_SANDBOX_IMAGE` / `COVEY_SANDBOX_IMAGE_DEV` überschreiben, worauf die beiden Profile auflösen. Die Regel beim Erweitern: **Version → Home, Toolchain → Image** — SDK-Versionen holt sich der Agent selbst in sein persistentes Home, dem Pin im Projekt-Repo folgend ([`docs/ops-deployment.md`](docs/ops-deployment.md)).
