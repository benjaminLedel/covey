...
Beide Namen vertragen ein `_Suffix` — `claude_code_oauth_token_team_a`, `anthropic_api_key_kostenstelle_b`. So hält eine Organisation mehrere Credentials nebeneinander, und jeder Agent wird unter *Secrets* am Agenten auf eines festgelegt (Rolle Platform Admin oder Security). Nicht festgelegte Agenten behalten die bisherige Reihenfolge: API-Key vor Abo-Token.
