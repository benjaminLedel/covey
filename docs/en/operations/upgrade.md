...
slug: upgrade
title: Upgrading
description: Migrations run by themselves at startup. This page lists the upgrades where that is not enough — where something has to be done by hand.
---
...
| `COVEY_ZAMMAD_INTAKE_GROUPS="Support L1"` | a **group condition on the Zammad trigger** ([`ops-zammad.md`](../integrations/zammad.md)) |
...
either; `dev` keeps working. [`ops-workplaces.md`](./workplaces.md) says
...
- **A second host**: `make runner`, then `covey-runner register` on that
  machine. See [`ops-runner.md`](./runner.md). The built-in runner keeps
...
- **Blocks in an object store**: `COVEY_BLOB_STORE=s3` (see
  [`ops-runner.md`](./runner.md)). Switching backends does **not** move the
