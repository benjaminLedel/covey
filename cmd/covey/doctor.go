...
// runDoctor answers, for THIS installation, what an upgrade or a restart is
// about to run into — before it does.
//
// Why a subcommand and not a page in the upgrade guide: a guide describes the
// general case, and the questions that matter are specific. Which images do MY
// agents need. What is MY database about to migrate. Where do MY blocks lie and
// how big are they. An answer somebody has to derive from a document by hand is
// one that gets derived wrongly, or not at all.
//
// It changes nothing. It reads, judges and names the remedy.
func runDoctor(ctx context.Context, cfg config.Config, args []string) error {
	...
	if blocking > 0 {
		...
		// a log nobody reads.
		return fmt.Errorf("%d of %d checks would keep agents from working", blocking, len(d.findings))
	}
	return nil
}
...
func (d *doctor) check(ctx context.Context) {
	pool, err := db.Connect(ctx, d.cfg.DatabaseURL)
	if err != nil {
