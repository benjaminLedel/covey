...
| [`28-team-surface.md`](28-team-surface.md) | The team surface (first slice built, the rest specified): the org chart as the directory rather than a second editor, humans in the conversation and how they are reached without a backlog, group conversations as an envelope that owns no work, the account-scoped reads several organisations at once would need — reading crosses the boundary, writing never does — and why a message should not automatically become a task |
