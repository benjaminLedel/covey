...
Der Runner ist ein **eigenes Release-Artefakt** (Binary je Architektur, Docker-Image, systemd-Unit, eigene Versionsnummer, Konfigurationsdatei mit Server-Adresse und Token) — er wird auf Maschinen installiert, auf denen sonst nichts von Covey liegt. Sein Code bleibt aber im **selben Repository**, nach dem Vorbild von `coveyd`: auch das ist ein eigenes Binary auf einer fremden Maschine, und `internal/daemon` definiert sein Protokoll für beide Seiten. So bleibt eine Protokolländerung ein einziger Commit statt eines Kompatibilitätstanzes über zwei Repositories. Ein getrenntes Repository bleibt möglich, sobald das Protokoll steht; für den Operator ändert es nichts, weil er das Artefakt sieht und nicht das Repository. Ein Runner-Prozess bedient dabei **genau eine** Covey-Instanz — ein über zwei Organisationen geteilter Blockspeicher wäre ein Kanal zwischen ihnen.
