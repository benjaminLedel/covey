...
Entscheidend ist dabei die **Richtung der Liste**: Aufgezählt wird, was verworfen werden darf, nicht was gesichert wird. Ein Agent legt seine Zwischenstände und selbstgeschriebenen Werkzeuge quer im Home ab, nicht in einem dafür vorgesehenen Ordner — im vermessenen Home liegen neben den Transkripten 95 KB extrahierter Code, mehrere Auswertungs-JSONs und zwei Aufgaben-Verzeichnisse, alle direkt im Wurzelverzeichnis. Eine Positivliste („sichere `work/` und `uploads/`") verlöre das stillschweigend, und jedes künftige Verzeichnis gleich mit. Bei einer Negativliste ist der unbekannte Neuzugang automatisch geschützt; ein Fehler darin kostet Platz statt Arbeit. Verworfen werden darf nur, was das Image-Profil als ableitbar ausweist (D11 — es kennt seine Toolchains und deren Cache-Pfade), und ein Checkout nur dann, wenn sein Arbeitsbaum sauber und alles gepusht ist.
