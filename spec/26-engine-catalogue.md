...
**The URL is the organisation's, never an agent's.** The catalogue is fetched by the control plane and by runners, on host privilege; content is verified by digest, but content is still what an organisation chose to trust. `COVEY_ENGINE_CATALOG_URL` is therefore an operator setting, read at process start, and nothing on an agent, a bundle or a task can point it somewhere else. An installation with its own registry or mirror sets its own URL and is right without editing this repository — as with the other catalogues, the default is derived from the source address (`buildinfo.SourceRepo`).
