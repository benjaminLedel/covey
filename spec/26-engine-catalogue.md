...
Published since, and consulted by default: `internal/engines/engine-catalog.json` is the document, the `catalog` branch serves the copy (§ Trust), and the address an unset `COVEY_ENGINE_CATALOG_URL` falls back to is that one. The release the document carries today sits behind a login, and the token for it belongs to the host that downloads — the runner, not the control plane, which only reads the document. `covey doctor` and the engine picker say whose host is missing which variable; whether the runner has it, a control plane on another machine cannot see and says so.
