...
**Addendum — the role workplaces.** The question only arises where an agent's field is open, and that is what `dev` answers. Where it is settled, the cut goes along the **role**: `dev-flutter`, `dev-php` and `dev-web` beside `dev`, which stays the default and the fallback. Nothing decides it automatically — which workplace an agent gets is a statement about the agent. And this is the one place where "version → home" is deliberately reversed: for a role the version is settled too, so `dev-flutter` carries its baseline SDK in the image rather than once per agent in the home, which is walked at every wake and written back after every run ([`16-runner.md`](16-runner.md)). Built: `agents.sandbox_image` carries either a profile name or an image of the organisation's own, and `COVEY_SANDBOX_IMAGE` is what a value-less agent falls back to. On a runner the image is at the same time a statement of capacity — it reports which ones it holds and gets only matching agents ([`16-runner.md`](16-runner.md)).
...
Decided along with it: **the runner protocol is the only way a sandbox starts, and by default the control plane speaks it with a runner it runs itself.** The single-machine installation therefore sets up nothing at all, and the remote path is not a second code path that only whoever operates two hosts ever exercises — it is the same one with a different transport. And: a runner belongs to **exactly one organisation**, inherited from the registration token; that holds for the built-in one, which exists once per organisation — and only as long as that organisation has registered none of its own. Registering the first runner makes the built-in one drain and stand down, deleting the last one brings it back. Otherwise there would be a mixed pool in which the control plane's host stays a compute node on the quiet.

The runner is a **release artefact of its own** (a binary per architecture, a Docker image, a systemd unit, its own version number, a configuration file with the server address and token) — it is installed on machines where nothing else of covey lives. Its code stays in the **same repository**, though, modelled on `coveyd`: that too is a separate binary on a foreign machine, and `internal/daemon` defines its protocol for both sides. That way a protocol change stays a single commit instead of a compatibility dance across two repositories. A separate repository stays possible once the protocol has settled; for the operator it changes nothing, because they see the artefact, not the repository. One runner process serves **exactly one** covey instance and there exactly one organisation — block storage shared across two organisations would be a channel between them.
