...
Related: [`20-hiring-and-setup.md`](20-hiring-and-setup.md) (**read that first** — the draft state, the
People department and the hiring brief; this document is the requirements-and-proof half of the same
story), [`22-org-templates.md`](22-org-templates.md) (a whole team at once),
[`02-agent-model.md`](02-agent-model.md) (config as code), [`04-identity-secrets.md`](04-identity-secrets.md)
...
**How this relates to hiring.** [`20`](20-hiring-and-setup.md) answers *what should this colleague be* —
a brief, an LLM-drafted config, a draft agent for a human to review. This document answers the question
that comes immediately after and is currently answered by nobody: *what does it need before it can
actually work, and how do we know it does?* A drafted agent whose `ACCESS.md` names `gitlab` still needs
a token, an egress allowlist and an enabled plugin, and the hiring flow deliberately does not grant
those — access is *requested* down the approval path. That request is exactly a requirement, and today
it is prose.
...
## 9. Verification feeds the hire decision — it is not a second gate

An agent that should not run yet already has a state: the **draft** from
[`20-hiring-and-setup.md`](20-hiring-and-setup.md) — `hired_at IS NULL`, not dispatched, no
heartbeat, no live webhook, no sandbox, no cost — and *every* path that produces an agent goes
through it, template instantiation and bundle import included.

This document therefore introduces **no gate of its own.** An earlier draft proposed `verified_at`
suppressing scheduled wakes; that was written before the draft state existed and would have put two
different facts — "nobody hired this yet" and "nobody checked this lately" — into two competing
mechanisms with one visible effect. Two ways for an agent to be silently not-running is one too many.

What the functional test does instead is **feed the decision that already exists.** The hire screen is
deliberately a summary rather than a confirmation dialog: the role, the target systems it asks for with
their scopes, the supervisor, the runtime, the budget ceiling. A verification result belongs in exactly
that list. This is the one point where a human takes responsibility for a new employee, and *"its
credentials resolve, its egress covers what its playbooks call, its sandbox has the toolchain"* is the
most useful sentence to have in front of you while doing it. A red T1–T3 does not block the hire — a
person may knowingly hire an agent whose token arrives tomorrow — it just means nobody can claim
afterwards that it looked fine.

For an **already-hired** agent a failed re-verification is a finding, never a silencer: the dispatcher
records an observability event and the fleet view flags it, and the agent keeps working. Guard rails
are fail-closed because an unevaluated policy must never permit an action; a verification result is a
statement about the last time someone looked, and a stale statement must not switch off a fleet that is
doing its job.

`verified_at` therefore records **when the platform last checked**, and carries no scheduling
semantics at all.
...
- **D1 — Declaration vs derivation on conflict.** Recommended: a declaration may add or narrow a
  ...
  for the audit trail.
- **D2 — Gating strictness. Resolved, by deletion.** The draft state in
  [`20-hiring-and-setup.md`](20-hiring-and-setup.md) already covers "exists but must not run", so
  verification needs no gate of its own and gets none (§9). Nothing left open.
...
- **D5 — Multi-agent templates. Resolved elsewhere:** [`22-org-templates.md`](22-org-templates.md)
  takes it, and reuses this document's requirement plan and verification tiers per member rather
