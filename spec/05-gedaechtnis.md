...
**Das Embedding trägt dieses Retrieval — und ist deshalb ein Port, kein Detail.** Der Built-in-Embedder (`HashEmbedder`) ist Feature-Hashing über Wörter und Bigramme: offline, deterministisch, ohne Abhängigkeiten — aber ein *lexikalisches* Maß. „Die Pipeline ist rot" und „Der CI-Build schlägt fehl" haben null gemeinsame Wörter und damit Ähnlichkeit 0. Damit greift die ganze Kette nicht: der Agent findet seine eigene Seite nicht wieder, sobald er anders formuliert, `Ingest` ordnet nichts zu, und der Konsolidierungs-Pass erkennt keine Duplikate. Für den Betrieb gehört deshalb ein echtes Embedding davor (`COVEY_EMBEDDING_PROVIDER`, siehe `internal/memory/embedder_api.go`). Zwei Wege, dieselbe Naht: **selbst betreiben** (`ollama`, ein kleines Modell auf dem eigenen Host — Default EmbeddingGemma, mehrsprachig, CPU-tauglich, Matryoshka-trainiert und damit auf die 256 Dimensionen des Schemas verkürzbar) oder ein **fremder Dienst** (`voyage`/`openai` + `COVEY_EMBEDDING_API_KEY`). Der selbstgehostete Weg ist der naheliegendere: Wiki-Seiten sind Kundenwissen, und so verlassen sie das Haus nicht. Der Built-in bleibt der Rückfall ohne beides.
