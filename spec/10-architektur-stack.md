...
Im UI erscheinen beide Arten (plus angebundene MCP-Server, kind=`mcp`) als **Store**: ein Katalog aller registrierten und hochgeladenen Plugins mit Suche, Kategorie-Filter und Aktivierung direkt auf der Karte, daneben eine Ansicht der aktiven Systeme. Die Kategorie (`ticketing`, `code`, `communication`, `files`, `web`, `dev`, `other`) deklariert das Plugin selbst — im `Descriptor` bzw. im Manifest-Feld `category`; die UI leitet ihre Filter aus den vorkommenden Kategorien ab und führt keine eigene Liste. Das Verhalten hängt nie an der Kategorie, sie ist reine Einordnung.

Beide Arten sind **pro Organisation aktivierbar** (Tabelle `target_plugins`); die Aktivierung ist **opt-in und fail-closed** — auch kompilierte Built-ins sind erst nach expliziter Aktivierung im UI nutzbar (Bestandsorganisationen wurden per Migration auf ihren bisherigen Stand gesetzt). Sie greift an den zentralen Enforcement-Punkten: der Webhook-Eingang lehnt nicht aktivierte Systeme ab, der Secrets-Broker gibt ihnen keine Credentials, und Prompt-Doku wie Profilfeld-Kennungen erscheinen nur für aktivierte Systeme. Credentials folgen der Konvention `<system>_token`/`<system>_url` im SecretStore.
...
- **Adresse erst zur Laufzeit:** Covey wird selbst gehostet, jede Installation hat ihre eigene Domain. Im vorgerenderten HTML steht deshalb ein Platzhalter, den der Server beim Ausliefern durch die tatsächliche Origin ersetzt — normalerweise die Herkunft des Requests, per `COVEY_SITE_URL` überschreibbar. `robots.txt` und `sitemap.xml` erzeugt das Binary aus demselben Grund selbst. Bewusst **nicht** `COVEY_PUBLIC_URL`: Die ist die von den Sandboxen erreichbare Adresse der Control Plane und damit eine interne Betriebsadresse — beides an eine Variable zu hängen hieße, dass niemand die Website-Adresse setzen kann, ohne die Data Plane vom Netz zu nehmen.
