...
**The second case has turned up, and with it the self-proposal.** Covey Doctor proposes changes to the configurations of colleagues it did not draft, and there the inactive version is not a gentler variant but the only defensible one — see [`21-operations-and-improvement.md`](21-operations-and-improvement.md). It is built, and allowing the self-proposal on top of it cost what was predicted here: nothing. `covey/propose_agent_config` is reachable with `agents:write` for the caller's **own** configuration; the People department may therefore sharpen its own role after the self-onboarding, as a stored version a human accepts. The rule above stands unchanged where it matters: it still may not *write* its own config, and `set_agent_config` still reaches only the drafts of the current assignment.
