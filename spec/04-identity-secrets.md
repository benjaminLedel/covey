...
- **deliberate** — a person **paused** the value by hand: the quota is needed elsewhere this week, the contract is being cancelled, the token is about to be rotated. Unlike the other two it is not a measurement, has no end the platform knows of, and nothing but a person lifts it — which is why it is kept apart from the cooldown rather than expressed as one far in the future.

All three take the value out of play; the agent then takes the least loaded healthy one (at equal load: the one fewest agents sit on) and keeps its previous seat as its **home**, so it returns there once that value is healthy again instead of the pool redistributing at every choice. Is **no** value usable, the broker refuses with the moment the pool frees up again (none, when every value is paused — nobody knows when a person resumes one), and the control plane **postpones** the wake rather than starting a run that cannot work — a rate limit thereby becomes a delay and not a failed task.
