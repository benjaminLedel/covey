...
### The later state wins, and the runner has to be able to tell which that is

Two states meet on a wake: the snapshot the control plane holds and the working copy on the host. Usually they are the same state and materialising touches nothing. When they differ, materialising brings every file the snapshot knows to *its* content — changed files included; only files the snapshot does not know are spared. That is a restore when the snapshot is the later state, and a reversal when the copy is: a sync whose answer never reached the control plane (the connection dropped while the runner finished it), or a run whose sync failed, leaves the copy ahead of the database, and the next wake used to write the run away.
...
- **The sleep path gets longer.** The sync runs at real falling-asleep — the compute goes down, then the scan runs on a home nothing writes into any more. A warm sandbox ([`03-lifecycle-scheduling.md`](03-lifecycle-scheduling.md)) never reaches that point, so it is synced *while parked*, in the background and at most once every five minutes: the agent counts as asleep the moment it is parked, and does not scan its home again on a two-minute heartbeat. Leaving the warm session untouched, as the first version did, was the cheaper answer and the wrong one — the work of a run then existed in one container volume on one host until something tore that sandbox down, and every way of losing the container in between (a hard restart, a prune, an OOM kill) lost the run with it.
