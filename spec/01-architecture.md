...
An agent may receive an opt-in Docker-in-Docker daemon for branch-local integration tests. That capability is a different trust class from ordinary sandbox processes: the daemon sidecar is privileged, ephemeral and reachable only through an internal per-agent API network. Under hard egress isolation its second network contains only that daemon and the allowlist proxy; it never joins the shared sandbox network. Nested workloads have no direct internet route and must receive proxy variables explicitly when they need permitted HTTP(S) egress. Under cooperative isolation those variables remain advisory, including inside nested workloads. A startup reconcile removes sidecars and their per-agent networks left behind by an interrupted control-plane process.
