...
**Distribution:** the data plane does not have to live on the control plane's host. **Runners** are execution nodes modelled on GitLab runners: a process on an arbitrary host registers with a registration token, holds an outbound connection and gets sandboxes assigned to it. Every sandbox starts this way, including on a single machine — there the control plane runs a **built-in runner** of its own, so the normal installation sets nothing up and the remote path is the same code with a different transport. It runs beside the registered hosts and takes over when none of them fits; whoever wants no compute on this machine **pauses** it — a decision a person makes and can read back, rather than one inferred from the existence of another host. A runner belongs to exactly one organisation. This is possible because the data plane never needs to be reachable inbound anyway — the daemon dials out (see the daemon protocol below). The persistent home does not stand in the way: after every job it is synced as a whole into a central, content-addressed **home store** and materialised from there on wake — the same hybrid pattern as the wiki ([`05-memory.md`](05-memory.md)), just without its restriction to curated content. Because deduplication happens block-wise, only changed blocks travel, and the toolchain caches that are identical across all agents are stored once org-wide. A home on a runner is therefore a disposable working copy, and the agent↔runner assignment a scheduling preference rather than a binding. Full model in [`16-runner.md`](16-runner.md).
...
| `cost` | Tokens/compute consumed, for budget tracking. The input side travels in three parts — fresh input, cache read, cache write — because with a caching runtime the uncached share is the smallest of the three by orders of magnitude (see [`06-observability-control.md`](06-observability-control.md)) |
...
**Two doors, one room.** The proxy serves the same actions twice: as plain HTTP (`POST /actions/<system>/<action>`, which the runtime calls with `curl`) and as an **MCP tool server** (`POST /mcp`, one tool per granted target system plus `covey`, the plugin's own `PromptDoc` as the description). The second door exists because the first one runs through a shell: the model writes JSON into a quoted shell string, and every quote, brace and redirection in a parameter is an opportunity to get it wrong — a wrong call is a wasted turn against the runaway guard. Both doors lead through **one** code path, so the guard-rail decision, the brokered execution and the recording stay in one place; a second path would eventually decide differently. It is deliberately one tool per system rather than one per action: the plugins describe their actions in prose, and a JSON schema per action would be a second truth drifting from the first. Which route a runtime takes is its adapter's business (for Claude Code `--mcp-config`, see [`12-claude-code-adapter.md`](12-claude-code-adapter.md)).
