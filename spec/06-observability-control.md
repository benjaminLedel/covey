...
- **Two cost kinds, kept apart.** What the runtime reports per run is real money on a metered credential (an API key) and **notional** on a subscription seat — there the amount is not billed, the seat is paid for regardless. A subscription therefore carries a **fixed** cost per period, spread across the runs that used it, and the reported per-run figure serves only as the relative weight for that spread. Adding notional to real overstates the bill by whatever share the subscriptions carry, silently, and it does so in exactly the figures controlling looks at. Every total has to say which of the two it means.
