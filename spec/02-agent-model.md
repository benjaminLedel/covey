...
The exact file list is deliberately extensible — further sensible MD files (e.g. `TONE.md`, `ESCALATION.md`) may be added. `TONE.md` has one special property: besides the prose that describes the voice, it may carry a ` ```style-profile ` block — the measured half of a style profile (bands, corpus values, lexicon, schema `covey-style/1`, produced by the covey-style skill from an author's corpus). The prose is compiled into the prompt like every other file; the block is stripped from the prompt and read by the **style gate** instead ([`06-observability-control.md`](06-observability-control.md)), which measures the text an agent is about to send against it. The model reads the voice, the platform reads the numbers. Core rule: **`ACCESS.md` contains references, not secrets.** Alongside these files, which every run carries in full, there are **skills** for procedures that load only on demand (see below).
