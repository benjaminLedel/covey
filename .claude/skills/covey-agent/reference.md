...
```json
{
  ...
  "agent": {
    ...
    "effort": "",                         // optional, empty = the runtime default; the levels are declared by the
                                          // engine (claude-code: low|medium|high|xhigh|max) — a level its engine
                                          // does not know is rejected on import
