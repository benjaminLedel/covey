...
- **Issues and commit messages are public, and carry nothing private.** They describe the product, not the people or machines around it:
  - no names, quotes or wishes of the people in the conversation — write what was observed and what should change, not who said it ("the owner asked for …", a quoted sentence from the chat);
  - no data from a real installation — the size or structure of an organisation, agent, customer or ticket names, anything read off a live or local instance;
  - no hosts, IP addresses, device or machine names, account, team or bundle-signing IDs, e-mail addresses, keys or tokens.

  Reproductions use made-up values (`192.168.x.x`, `ada`, `example.org`). An issue edited afterwards keeps its old text in GitHub's edit history, so the text has to be right before it is posted, not corrected after.
