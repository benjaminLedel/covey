...
func TestIndicatorsCountObjectsNotEvents(t *testing.T) {
	...
	if err := s.obs.AddCost(ctx, agent.ID, &task.ID, 6.00,
		observability.Tokens{Output: 500}, "claude-opus-5", "", 0); err != nil {
		t.Fatal(err)
	}
...
func TestIndicatorsGroupByKeyAndCountFailures(t *testing.T) {
	...
	for _, slug := range []string{"support-a", "support-b"} {
		...
		if err := s.obs.AddCost(ctx, a.ID, &task.ID, 3.00, observability.Tokens{}, "claude-opus-5", "", 0); err != nil {
			t.Fatal(err)
		}
	}
	...
	if err := s.obs.AddCost(ctx, stumm.ID, &kaputt.ID, 90.00, observability.Tokens{}, "claude-opus-5", "", 0); err != nil {
		t.Fatal(err)
	}
