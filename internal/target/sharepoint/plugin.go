...
Details: docs/betrieb-sharepoint.md im Repository.`,
	})
}
...
func (System) PromptDoc() string {
	return `Verfügbare SharePoint-/Teams-Datei-Aktionen (eine dir freigegebene Dokumentbibliothek; alle Pfade
   relativ zu deren Wurzel): list {"path":"unterordner (optional, leer = Wurzel)"} listet Dateien und Ordner,
   read {"path":"a/b.txt"} liefert den Inhalt einer Textdatei direkt (bis 1 MB, nur Text),
   write {"path":"a/b.txt","content":"..."} legt eine Textdatei an oder überschreibt sie,
   download {"path":"a/bericht.docx","to":"lokaler/pfad (optional)"} holt eine Datei in deine Sandbox
   (Default: sharepoint/<pfad>) und liefert den lokalen Pfad,
   upload {"from":"lokaler/pfad","to":"remote/pfad (optional, Default: Dateiname in der Wurzel)"} legt eine
   Datei aus deiner Sandbox in der Bibliothek ab (ersetzt Vorhandenes),
   ...
   WARTEN auf neue Dateien: SharePoint hat hier keinen Webhook — nutze KEINEN blocked-Status; beende deinen
