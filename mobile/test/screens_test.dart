...
http.Response _json(Object body) =>
    http.Response.bytes(utf8.encode(jsonEncode(body)), 200, headers: {'content-type': 'application/json; charset=utf-8'});
...
final _thread = {
  'pending': false,
  'entries': [
    {'kind': 'message', 'id': 't1', 'task_id': 't1', 'task_title': 'Rechnung', 'author': 'chat:a@b.c', 'text': 'Bitte die Rechnung prüfen', 'task_state': 'blocked'},
    {'kind': 'question', 'id': 'n1', 'task_id': 't1', 'task_title': 'Rechnung', 'author': 'agent', 'text': 'Darf ich Globex direkt antworten?', 'task_state': 'blocked'},
  ],
};
...
void main() {
  testWidgets('the connect screen refuses plain http before asking anything', (tester) async {
    ...
    await tester.pumpWidget(await tester.runAsync(() => _app(ConnectScreen(
          ...
          makeApi: (b, k) => CoveyApi(b, k, client: MockClient((_) async {
            asked = true;
  ...
  testWidgets('a reply goes to the question it was chosen at', (tester) async {
    ...
    await tester.pumpWidget(await tester.runAsync(() => _app(ThreadScreen(api: api, agentId: 'a1', agentName: 'Bea', me: _me()))) as Widget);
  ...
  testWidgets('without the team surface the thread can be read and not written to', (tester) async {
    final api = CoveyApi(Uri.parse('https://c.example'), 'k',
        client: MockClient((_) async => _json(_thread)));
    await tester.pumpWidget(
        await tester.runAsync(() => _app(ThreadScreen(api: api, agentId: 'a1', agentName: 'Bea', me: _me(team: false)))) as Widget);
    await _settle(tester);
